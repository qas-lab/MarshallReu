...
def breakTies(devs, probs):

    max_prob = max(probs)
    tied_devs = [dev for dev, prob in zip(devs, probs) if prob == max_prob]
    
    # For simplicity, return the first tied developer
    return tied_devs[0] if tied_devs else None
...
if len(testingDoc) > 0:
    ...
    for i in range(num):
        ...
        if filtered_docDictionary:
            ...
            best_dev = breakTies(top_5_devs, top_5_probs)
